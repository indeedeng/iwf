...
idl-code-gen: #generate/refresh go clent code for idl, do this after update the idl file
	rm -Rf ./gen ; true
	openapi-generator generate -i iwf-idl/workflowState.yaml -g go -o gen/client/workflow/state/ -p packageName=state -p generateInterfaces=true -p isGoSubmodule=false --git-user-id cadence-oss --git-repo-id iwf-idl
	rm ./gen/client/workflow/state/go.* ; true
	openapi-generator generate -i iwf-idl/workflow.yaml -g go-gin-server -o gen/server/ -p packageName=workflow -p apiPath=workflow  --git-user-id cadence-oss --git-repo-id iwf-idl
	rm ./gen/server/main.go ; rm ./gen/server/workflow/api_default.go ; rm ./gen/server/workflow/routers.go ; true
