...
// WaitForAllUserAndSystemSignalsToReceive will wait for all signals are received before a safe continueAsNew
...
func (sr *SignalReceiver) WaitForAllUserAndSystemSignalsToReceive(ctx UnifiedContext) error {
	return sr.provider.Await(ctx, func() bool {
		...
		for _, sigName := range unhandledSigs {
			...
			// found a regular signal, return false so that continueAsNew can wait for it
			return false
		}
		// no unhandled signals
		return true
	})
}
