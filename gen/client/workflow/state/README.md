...
```golang
import state "github.com/cadence-oss/iwf-idl"
```
...
```golang
ctx := context.WithValue(context.Background(), state.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), state.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```
ctx := context.WithValue(context.Background(), state.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), state.ContextOperationServerVariables, map[string]map[string]string{
